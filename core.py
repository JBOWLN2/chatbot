...
def greet():
    ...
    will_help = \
        input("Hi {}, I'm Etcetera. I'm not from this planet. Will you help me learn about your planet? ".format(name))
...
def alienbot():
    if greet() is True:
        while True:
            ...
            if make_exit(reply) is True:
                break
            answer = converse(reply)
            print(answer)
    ...
    for pair in alienbabble:
        for (regex_pattern, alien_answers) in pair.items():
            ...
            if found_match:
                ...
                formatted_alien_answer = \
                    alien_answer.format(*[reflect(matching_group)
                        for matching_group in found_match.groups()])
                return formatted_alien_answer
